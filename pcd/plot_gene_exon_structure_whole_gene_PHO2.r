...
sss_n <- length(samplelist)
	...
	for(i in 1:dim(Control)[1])
	{
		...
		add <- rnorm(1, mean = 0.3, sd = 0.1)
		add <- 0.2
		text(mid, 1.5+add, tmpin, cex=1.2)
		lines(c(target_st, mid), c(1.5, 1.5-0.1), lwd=1)
		lines(c(mid, target_en), c(1.5-0.1, 1.5), lwd=1)
		add <- rnorm(1, mean = 0.2, sd = 0.008)
		text((target_en+target_st)/2, 1.5-add, tmpex, cex=1.2)
	}
...
par(mar=c(4,12,0,3))
gtf1 <- read.table("tid1.gtf", sep="\t", header=FALSE)
tmp1 <-  strsplit(unique(as.vector(gtf1$V9)), "\\;\\s")
tmp1 <- tmp1[[1]][2]
tmp1 <- sub("transcript_id ", "", tmp1)
tmp1 <- sub("\\;", "", tmp1)

gtf2 <- read.table("tid2.gtf", sep="\t", header=FALSE)
...
gtf3 <- read.table("tid3.gtf", sep="\t", header=FALSE)
tmp3 <-  strsplit(unique(as.vector(gtf3$V9)), "\\;\\s")
tmp3 <- tmp3[[1]][2]
tmp3 <- sub("transcript_id ", "", tmp3)
tmp3 <- sub("\\;", "", tmp3)



tids <- c(tmp1, tmp2, tmp3)

pos <- rbind(gtf1[,c("V4", "V5")],gtf2[,c("V4", "V5")], gtf3[,c("V4", "V5")])
gst <- min(pos)
gen <- max(pos)
...
#dev.off()
gtf <- read.table("tid1.gtf", sep="\t", header=FALSE)
gtf <- gtf[order(gtf$V4), ]
i=4
source(fun1)
gtf <- read.table("tid2.gtf", sep="\t", header=FALSE)
gtf <- gtf[order(gtf$V4), ]
i=3
source(fun1)
i=2
gtf <- read.table("tid3.gtf", sep="\t", header=FALSE)
gtf <- gtf[order(gtf$V4), ]
source(fun1)
...
if(is.element('PF00179', exon_type_uniq))
{
legend1 <- c(legend1, 'Ubiquitin-conjugating enzyme')

...
use Getopt::Long;
use vars qw($gtffile $A3SSeventfile $bedfile);
Getopt::Long::GetOptions(
	'gtf=s'    => \$gtffile,
	'A3SS=s'     => \$A3SSeventfile,
);
...
while(<IN>)
{
	...
	if($target_strand eq "+")
	{
		...
		my $target_down_exst=$elements[6]+1;
		$target_down_exst++;
		...
		foreach my $key (keys %transcript)
		{
			...
			if(scalar(@exsst) >= 2)
			{
				for(my $pos1 = 0; $pos1 <= ($#exsst - 1); $pos1++)
				{
					...
					if($left_overlap != 12 and $right_overlap != 12 and $middle_overlap != 12 and $two1en[0] eq $target_up_exen and $two1st[1] eq $target_exst)
					{
						#print $tid."\n";
		...
		foreach my $key (keys %transcript)
		{
			...
			if(scalar(@exsst) >= 2)
			{
				for(my $pos1 = 0; $pos1 <= ($#exsst - 1); $pos1++)
				{
					...
					my $right_overlap = overlap($target_down_exst, $target_down_exen, $two1st[1], $two1en[1]);
					my $middle_overlap = overlap($target_exst, $target_exen, $two1st[0], $two1en[0]);
					if($left_overlap != 12 and $right_overlap != 12 and $middle_overlap == 12 and ($two1en[0]+1) eq $target_exst and $two1st[1] eq $target_down_exst)

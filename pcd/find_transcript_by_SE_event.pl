...
use Getopt::Long;
use vars qw($gtffile $SEeventfile);
...
#$tt=~s/\n$//;
#$tt=~s/\s+$//;
print "Event"."\t"."Inclusive_transcript_ids"."\t"."Exclusive_transcript_ids"."\n";
while(<IN>)
{
    chomp;
	s/\s+$//;
	...
	foreach my $key (keys %transcript)
	{
		...
		if(scalar(@exsst) >= 3)
		{
			for(my $pos1 = 0; $pos1 <= ($#exsst - 2); $pos1++)
			{
				...
				if( $left_overlap != 12 and $right_overlap != 12 and $target_up_exen eq $trio1en[0] and $target_down_exst eq $trio1st[2] and $trio1st[1] eq $target_exst and $trio1en[1] eq $target_exen)
				{
				    #print $tid."\n";
		...
		if(scalar(@exsst) >= 2)
		{
			for(my $pos1 = 0; $pos1 <= ($#exsst - 1); $pos1++)
			{
				...
				if($left_overlap != 12 and $right_overlap != 12 and $target_up_exen eq $two1en[0] and $target_down_exst eq $two1st[1])
				{
				    #print $tid."\n";
